...
void CalcOnFPGA::SendDatatoFPGA(const size_t N,
  ...
  status = clEnqueueWriteBuffer(command_queue, COL_IND_buf, CL_FALSE, 0, sizeof(int)*VAL_SIZE, COL_IND, 0, NULL, &write_event[0]);
  aocl_utils::checkError(status, "Failed to transfer input COL_IND");
  status = clEnqueueWriteBuffer(command_queue, ROW_PTR_buf, CL_FALSE, 0, sizeof(int)*N, ROW_PTR, 0, NULL, &write_event[0]);
